# Telegram Mini App: Демо-казино

Полноценный пример Telegram Mini App с виртуальными играми: Plinko, Lucky Jet и слоты.
...
| POST  | `/api/plinko/bet` | Сделать ставку в Plinko |
| POST  | `/api/lucky/spin` | Сделать ставку в Lucky Jet |
| POST  | `/api/slots/spin` | Крутить слоты |
...
- Случайные результаты игр генерируются на сервере (`secrets.randbelow`).
- Баланс и история хранятся в SQLite, обновляются только через API.

## 9. Возможные доработки

- Добавить блэкджек, кости.
